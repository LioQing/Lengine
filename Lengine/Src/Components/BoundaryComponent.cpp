...
BoundaryComponent::BoundaryComponent(Matrixi map, int wall_i, int side_wall_i, uint32_t size_x, uint32_t size_y, uint32_t tile_size)
	: tile_size(tile_size)
{
	LoadBoundary(map, wall_i, side_wall_i, size_x, size_y);
}

void BoundaryComponent::LoadBoundary(Matrixi map, int wall_i, int side_wall_i, uint32_t size_x, uint32_t size_y)
{
	...
	for (auto y = 0u; y < size_y; ++y)
	{
		...
		{
			if (map.At(x, y) == wall_i || map.At(x, y) == side_wall_i)
			{
