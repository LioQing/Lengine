		...
		for (auto& e : enemies)
		{
			...
			if (game.load()->InsideView(sf::FloatRect(e_pos.sfVector2f(), sf::Vector2f(0.f, 0.f))) && CanSeePlayer(enemy))
			{
				if (*e->gun_pt_dir.load() != (*player_pos.load() - e_pos).Normalize())
				{
					...
					if (Vec2<float>::Zero().Orientation(*e->gun_pt_dir.load(), player_dir) != orientation)
					{
						*e->gun_pt_dir.load() = player_dir;
					}
				}
			...
			{
				// update gun pt dir
				if (*e->movement.load() != Vector2Df::Zero() && *e->gun_pt_dir.load() != *e->movement.load())
				{
